...
FROM 
(select selection_content.id_row, cast(column_value as nchar) as column_value, dsv_columns.column_name from selection_content INNER JOIN 
	...
	models ON models.id_structure = structures.id_structure AND models.name =  'mod_1'
						) p
PIVOT
(
max(column_value)
